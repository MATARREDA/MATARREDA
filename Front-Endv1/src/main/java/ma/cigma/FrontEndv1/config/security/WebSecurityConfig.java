package ma.cigma.FrontEndv1.config.security;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/add-client").hasRole("USER")
				.antMatchers("/add-client", "/update/**", "/delete-client/**").hasRole("ADMIN")
				.antMatchers("/clients", "/").permitAll().anyRequest().authenticated().and().formLogin()
